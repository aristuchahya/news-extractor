...
def parse(soup: BeautifulSoup, url: str) -> dict:
    ...
    author_el = soup.select_one(".detail__author, .author")
    category_el = soup.select_one(".breadcrumb a:last-of-type, .nav__list a")
    tag_els = soup.select(".detail__body-tag a, .nav-tag__item a")
